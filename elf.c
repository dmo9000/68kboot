...
int elf_load_binary(int elf_fd)
{
...
#ifdef DEBUG_ELF
    kprintf("[elf.c(elf_fd=%d): read %d bytes]\r\n", elf_fd, rd);
#endif
    ...
    if ((addr[0] == 0x7f) && (addr[1] == 'E') &&
            (addr[2] == 'L') && (addr[3] == 'F')) {
        //kprintf("ELF executable found at 0x%lx\r\n", addr);
    } else {
        //kprintf("ELF executable _NOT_ found at 0x%lx\r\n", addr);
        return (0);
    }
    ...
    case ELFDATA2LSB:
        if (is_big_endian()) {
            kprintf("elf.c: wrong byte order (LSB)\r\n");
            return 0;
        }
        break;
    case ELFDATA2MSB:
        if (! is_big_endian()) {
            kprintf("elf.c: wrong byte order (MSB)\r\n");
            return 0;
        }
        break;
    default:
        kprintf("elf.c: unsupported elf->e_ident value (%u)\r\n", Header->e_ident[EI_DATA]);
        return 0;
    }
...
//		assert(NULL);
    ...
    for (i = 0; i < num_entries; i++) {
        ...
        //memcpy(&sh_strname, sh_string, kstrlen(sh_string));
...
#ifdef DEBUG_ELF
    kprintf("[ELF LOADER RETURNING 1]\r\n");
    kernel_puts("\r\n");
#endif
    return(1);
}
