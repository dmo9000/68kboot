...
//1            printf("running sum(0x%lx, %d, '%s')\r\n", fd, fileno(fd), argv[c]);
            sum(fd, argv[c]);
            fclose(fd);
        }
    }
...
//        printf("-> sum(0x%lx), fileno(fd) = %d\r\n", fd, fileno(fd));
//        printf("calling fread(0x%lx, 1, %u, 0x%lx(%d)\r\n", buf+n, 128*62-n, fd, fileno(fd));
        i = fread(buf+n, 1, 128*64-n, fd);
//        printf("%d = fread(0x%lx, 1, %u, 0x%lx(%d))\r\n", i, buf+n, 128*62-n, fd, fileno(fd));
//        puts("\r\n");
        if(i <= 0)
            break;
        n += i;
